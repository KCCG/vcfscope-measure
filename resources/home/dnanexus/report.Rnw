...
report$specifications$label = sprintf("%s >= %3g", report$specifications$measure, report$specifications$cutoff)
report$specifications$label_latex = sprintf("$\\mathrm{%s} \\geq %3g$", report$specifications$measure, report$specifications$cutoff)
...
Call criterion: \Sexpr{report$specifications$label_latex} \\
...
plotROC(perf.snv$zyg[[report$specifications$measure]][c("RRvsAA", "RRvsRA")]) + 
	...
	coord_cartesian(xlim = c(-1e-7, 1e-5)) + ylim(0, 1) + 
	geom_point(aes(colour = Zygosity, shape = Score), size = 4, data = data.frame(
		FP = c(1 - snv.het.perf$spec, 1 - snv.hom.perf$spec, 1 - snv.het.perf_FILTER$spec, 1 - snv.hom.perf_FILTER$spec),
		TP = c(snv.het.perf$sens, snv.hom.perf$sens, snv.het.perf_FILTER$sens, snv.hom.perf_FILTER$sens),
		Zygosity = c("RRvsRA", "RRvsAA", "RRvsRA", "RRvsAA"),
		Score = c(report$specifications$label, report$specifications$label, "FILTER = PASS", "FILTER = PASS"))) + 
	scale_shape_manual(name = "Score", breaks = c(report$specifications$label, "FILTER = PASS"), values = c(5, 16)) + 
	guides(size = FALSE) + 
...
plotROC(perf.indelsubst$zyg[[report$specifications$measure]][c("RRvsAA", "RRvsRA")], type.fp = "count") + 
	...
	ylim(0, 1) + 
	geom_point(aes(colour = Zygosity, shape = Score), size = 4, data = data.frame(
		FP = c(indelsubst.het.perf$nfp, indelsubst.hom.perf$nfp, indelsubst.het.perf_FILTER$nfp, indelsubst.hom.perf_FILTER$nfp),
		TP = c(indelsubst.het.perf$sens, indelsubst.hom.perf$sens, indelsubst.het.perf_FILTER$sens, indelsubst.hom.perf_FILTER$sens),
		Zygosity = c("RRvsRA", "RRvsAA", "RRvsRA", "RRvsAA"),
		Score = c(report$specifications$label, report$specifications$label, "FILTER = PASS", "FILTER = PASS"))) + 
	scale_shape_manual(name = "Score", breaks = c(report$specifications$label, "FILTER = PASS"), values = c(5, 16)) + 
	guides(size = FALSE) + 
...
plotROC(perf.snv.coding10$zyg[[report$specifications$measure]][c("RRvsAA", "RRvsRA")]) + 
	...
	coord_cartesian(xlim = c(-1e-7, 1e-5)) + ylim(0, 1) + 
	geom_point(aes(colour = Zygosity, shape = Score), size = 4, data = data.frame(
		FP = c(1 - snv.coding10.het.perf$spec, 1 - snv.coding10.hom.perf$spec, 1 - snv.coding10.het.perf_FILTER$spec, 1 - snv.coding10.hom.perf_FILTER$spec),
		TP = c(snv.coding10.het.perf$sens, snv.coding10.hom.perf$sens, snv.coding10.het.perf_FILTER$sens, snv.coding10.hom.perf_FILTER$sens),
		Zygosity = c("RRvsRA", "RRvsAA", "RRvsRA", "RRvsAA"),
		Score = c(report$specifications$label, report$specifications$label, "FILTER = PASS", "FILTER = PASS"))) + 
	scale_shape_manual(name = "Score", breaks = c(report$specifications$label, "FILTER = PASS"), values = c(5, 16)) + 
	guides(size = FALSE) + 
...
plotROC(perf.indelsubst.coding10$zyg[[report$specifications$measure]][c("RRvsAA", "RRvsRA")], type.fp = "count") + 
	...
	ylim(0, 1) + 
	geom_point(aes(colour = Zygosity, shape = Score), size = 4, data = data.frame(
		FP = c(indelsubst.coding10.het.perf$nfp, indelsubst.coding10.hom.perf$nfp, indelsubst.coding10.het.perf_FILTER$nfp, indelsubst.coding10.hom.perf_FILTER$nfp),
		TP = c(indelsubst.coding10.het.perf$sens, indelsubst.coding10.hom.perf$sens, indelsubst.coding10.het.perf_FILTER$sens, indelsubst.coding10.hom.perf_FILTER$sens),
		Zygosity = c("RRvsRA", "RRvsAA", "RRvsRA", "RRvsAA"),
		Score = c(report$specifications$label, report$specifications$label, "FILTER = PASS", "FILTER = PASS"))) + 
	scale_shape_manual(name = "Score", breaks = c(report$specifications$label, "FILTER = PASS"), values = c(5, 16)) + 
	guides(size = FALSE) + 
...
\subsection{Raw Counts at Cutoff: \Sexpr{report$specifications$label_latex}}
<<count-table,results='asis'>>=
count.table = xtable(data.frame(
	"Region" = c("Whole genome", "", "", "", "Coding $\\pm10$ bp", "", "", ""),
	"Zygosity" = c("Het", "", "Hom", "", "Het", "", "Hom", ""),
	"Mutation" = c("SNV", "Indel/MNV", "SNV", "Indel/MNV", "SNV", "Indel/MNV", "SNV", "Indel/MNV"),
	"TP" = c(
		formatC2(snv.het.perf$ntp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.het.perf$ntp, digits=0, format="f", big.mark=","),
		formatC2(snv.hom.perf$ntp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.hom.perf$ntp, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.het.perf$ntp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.het.perf$ntp, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.hom.perf$ntp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.hom.perf$ntp, digits=0, format="f", big.mark=",")),
	"FP" = c(
		formatC2(snv.het.perf$nfp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.het.perf$nfp, digits=0, format="f", big.mark=","),
		formatC2(snv.hom.perf$nfp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.hom.perf$nfp, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.het.perf$nfp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.het.perf$nfp, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.hom.perf$nfp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.hom.perf$nfp, digits=0, format="f", big.mark=",")),
	"TN" = c(
		formatC2(snv.het.perf$ntn, digits=0, format="f", big.mark=","),
		NA,
		formatC2(snv.hom.perf$ntn, digits=0, format="f", big.mark=","),
		NA,
		formatC2(snv.coding10.het.perf$ntn, digits=0, format="f", big.mark=","),
		NA,
		formatC2(snv.coding10.hom.perf$ntn, digits=0, format="f", big.mark=","),
		NA),
	"FN" = c(
		formatC2(snv.het.perf$nfn, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.het.perf$nfn, digits=0, format="f", big.mark=","),
		formatC2(snv.hom.perf$nfn, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.hom.perf$nfn, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.het.perf$nfn, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.het.perf$nfn, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.hom.perf$nfn, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.hom.perf$nfn, digits=0, format="f", big.mark=","))),
	digits = 0, align = "llllrrrr")
print(count.table, sanitize.text.function = function(x) x, include.rownames = FALSE, floating = FALSE)
@

\subsection{Raw Counts at Cutoff: $\mathrm{FILTER} = \mathrm{PASS}$}
<<count-table-FILTER,results='asis'>>=
count.table_FILTER = xtable(data.frame(
	"Region" = c("Whole genome", "", "", "", "Coding $\\pm10$ bp", "", "", ""),
	"Zygosity" = c("Het", "", "Hom", "", "Het", "", "Hom", ""),
	"Mutation" = c("SNV", "Indel/MNV", "SNV", "Indel/MNV", "SNV", "Indel/MNV", "SNV", "Indel/MNV"),
	"TP" = c(
		formatC2(snv.het.perf_FILTER$ntp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.het.perf_FILTER$ntp, digits=0, format="f", big.mark=","),
		formatC2(snv.hom.perf_FILTER$ntp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.hom.perf_FILTER$ntp, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.het.perf_FILTER$ntp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.het.perf_FILTER$ntp, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.hom.perf_FILTER$ntp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.hom.perf_FILTER$ntp, digits=0, format="f", big.mark=",")),
	"FP" = c(
		formatC2(snv.het.perf_FILTER$nfp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.het.perf_FILTER$nfp, digits=0, format="f", big.mark=","),
		formatC2(snv.hom.perf_FILTER$nfp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.hom.perf_FILTER$nfp, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.het.perf_FILTER$nfp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.het.perf_FILTER$nfp, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.hom.perf_FILTER$nfp, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.hom.perf_FILTER$nfp, digits=0, format="f", big.mark=",")),
	"TN" = c(
		formatC2(snv.het.perf_FILTER$ntn, digits=0, format="f", big.mark=","),
		NA,
		formatC2(snv.hom.perf_FILTER$ntn, digits=0, format="f", big.mark=","),
		NA,
		formatC2(snv.coding10.het.perf_FILTER$ntn, digits=0, format="f", big.mark=","),
		NA,
		formatC2(snv.coding10.hom.perf_FILTER$ntn, digits=0, format="f", big.mark=","),
		NA),
	"FN" = c(
		formatC2(snv.het.perf_FILTER$nfn, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.het.perf_FILTER$nfn, digits=0, format="f", big.mark=","),
		formatC2(snv.hom.perf_FILTER$nfn, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.hom.perf_FILTER$nfn, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.het.perf_FILTER$nfn, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.het.perf_FILTER$nfn, digits=0, format="f", big.mark=","),
		formatC2(snv.coding10.hom.perf_FILTER$nfn, digits=0, format="f", big.mark=","),
		formatC2(indelsubst.coding10.hom.perf_FILTER$nfn, digits=0, format="f", big.mark=","))),
	digits = 0, align = "llllrrrr")
print(count.table_FILTER, sanitize.text.function = function(x) x, include.rownames = FALSE, floating = FALSE)
@
