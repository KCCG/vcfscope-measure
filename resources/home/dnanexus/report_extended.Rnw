...
plotROC(perf.snv$all) + coord_cartesian(xlim = c(-1e-7, 1e-5)) + ylim(0, 1) + theme_bw() + scale_colour_brewer(palette = "Set1") + ggtitle("SNVs: Overview")
plotROC(perf.snv$all) + coord_cartesian(xlim = c(-1e-7, 2.5e-6), ylim = c(0.8, 1)) + theme_bw() + scale_colour_brewer(palette = "Set1") + ggtitle("SNVs: Zoom")
...
plotROC(lapply(perf.snv$zyg, function(x) x[c("RRvsAA", "RRvsRA")])) + 
	...
	theme_bw() + ggtitle("Zygosity: Overview")
plotROC(lapply(perf.snv$zyg, function(x) x[c("RRvsAA", "RRvsRA")])) + 
	coord_cartesian(xlim = c(-1e-7, 2.5e-6), ylim = c(0.8, 1)) + 
	labs(colour = "Score", linetype = "Zygosity") + 
	scale_colour_brewer(palette = "Set1") + 
	scale_linetype_manual(breaks = c("RRvsAA", "RRvsRA", "RRvsAB"), values = c("solid", "dashed", "blank")) + 
	theme_bw() + ggtitle("Zygosity: Zoom")

plotROC(lapply(perf.snv$mask, function(x) x[c("unmasked", "repetitive", "low_complexity")])) + 
	coord_cartesian(xlim = c(-1e-7, 1e-5)) + ylim(0, 1) + 
	...
	theme_bw() + ggtitle("Sequence type: Overview")
plotROC(lapply(perf.snv$mask, function(x) x[c("unmasked", "repetitive", "low_complexity")])) + 
	coord_cartesian(xlim = c(-1e-7, 2.5e-6), ylim = c(0.8, 1)) + 
	labs(colour = "Score", linetype = "Sequence mask") + 
	scale_colour_brewer(palette = "Set1") + 
	scale_linetype_manual(breaks = c("low_complexity", "repetitive", "unmasked"), values = c("dashed", "dotted", "solid")) + 
	theme_bw() + ggtitle("Sequence type: Zoom")
@
