...
summary.table = xtable(data.frame(
	...
	"Reference Range" = c(
		sprintf("$> %.2f$", report$specifications$snv.het.min.sens), 
		sprintf("$> %.2f$", report$specifications$snv.het.min.spec), 
		sprintf("$> %.2f", report$specifications$indelsubst.het.min.sens),
		sprintf("$> %.2f$", report$specifications$snv.hom.min.sens), 
		sprintf("$> %.2f$", report$specifications$snv.hom.min.spec), 
		sprintf("$> %.2f", report$specifications$indelsubst.hom.min.sens)),
	"Call" = c(
...
count.table = xtable(data.frame(
	...
	"TP" = c(
		snv.het.perf$tp,
		indelsubst.het.perf$tp,
		snv.hom.perf$tp,
		indelsubst.hom.perf$tp),
	"FP" = c(
		snv.het.perf$fp,
		indelsubst.het.perf$fp,
		snv.hom.perf$fp,
		indelsubst.hom.perf$fp),
	"TN" = c(
		snv.het.perf$tn,
		"---",
		snv.hom.perf$tn,
	...
	"FN" = c(
		snv.het.perf$fn,
		indelsubst.het.perf$fn,
		snv.hom.perf$fn,
		indelsubst.hom.perf$fn),
	digits = 4)
